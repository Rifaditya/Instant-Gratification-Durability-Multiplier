# 갓 모드 및 무한 내구도 (26.1.2)

| 시스템 매개변수 | 설정값 |
| :--- | :--- |
| **전체 무한 규칙** | `ig:dm_infinity_global` |
| **기본 상태** | `false` (비활성화) |
| **피해 차단** | `HEAD`에서 들어오는 피해 취소 ($0$ 피해 적용) |
| **툴팁 서식** | `✦ UNBREAKABLE` (금색, 볼드체) |
| **우선순위** | 절대 우선 (모든 배율 계산에 앞서 평가) |

---

## ⚡ 개요 및 메커니즘

**신 모드 (무한)**는 선택한 범주의 아이템에 완전한 무적 상태를 부여합니다. 신 모드가 활성화된 아이템은 `ItemStackDurabilityMixin`에서 내구도 소모 이벤트가 완전히 차단 및 취소되어 절대 피해를 입거나 파괴되지 않습니다.

### 바닐라 파괴 불가 컴포넌트와의 차이점
* 바닐라의 `Unbreakable` 컴포넌트는 명령어를 통해 개별 아이템 스택에 일일이 부여해야 합니다 (`/give @p diamond_sword[unbreakable={}]`).
* 신 모드는 **월드 전체 및 범주 전체**에 적용됩니다. NBT 수정 없이도 월드 내 모든 도구, 무기, 방어구가 자동으로 파괴 불가 상태가 됩니다.

---

## 🛡️ 24가지 갓 모드 규칙

| # | 게임 규칙 키 | 범주 이름 | 대상 아이템 | 기본값 |
| :-: | :--- | :--- | :--- | :-: |
...
`DurabilityHelper.isInfinite(ServerLevel, ItemStack)` 판정 순서:
...
신 모드는 모든 배율이나 1회용 설정에 대해 **절대적인 우선권**을 갖습니다. `ig:dm_infinity_tools = true`인 경우, 퍼센트 설정값과 무관하게 도구는 절대 피해를 입지 않습니다.
