# Modo Dios e infinidad (26.1.2)

| Parámetro del sistema | Valor |
| :--- | :--- |
| **Regla global de infinito** | `ig:dm_infinity_global` |
| **Estado predeterminado** | `false` (Desactivado) |
| **Intercepción de daño** | Daño entrante cancelado en `HEAD` ($0$ daño aplicado) |
| **Estilo de descripción emergente** | `✦ UNBREAKABLE` (Dorado, Negrita) |
| **Prioridad** | Absoluta (Evaluada antes de cualquier multiplicador) |

---

## ⚡ Descripción general y mecánicas

**Modo Dios (Infinito)** otorga invencibilidad completa a los objetos de las categorías seleccionadas. Cuando el Modo Dios está activo para un objeto, cualquier evento de pérdida de durabilidad se intercepta y cancela por completo en `ItemStackDurabilityMixin`, evitando que el objeto reciba daño o se rompa.

### Distinción con respecto al componente irrompible vanilla
* El componente `Unbreakable` de vanilla debe aplicarse a objetos individuales mediante comandos (`/give @p diamond_sword[unbreakable={}]`).
* El Modo Dios de Durability Multiplier es **a nivel de todo el mundo y categoría**: cada herramienta, arma o pieza de armadura en el mundo se comporta automáticamente como irrompible sin editar NBT ni componentes.

---

## 🛡️ Las 24 reglas del Modo Dios

| # | Clave de GameRule | Nombre de categoría | Objetos objetivo | Predeterminado |
| :-: | :--- | :--- | :--- | :-: |
...
## 👑 Orden de resolución del Modo Dios

`DurabilityHelper.isInfinite(ServerLevel, ItemStack)` comprueba:
...
El Modo Dios tiene **prioridad absoluta** sobre cualquier ajuste de multiplicador o un solo uso. Si `ig:dm_infinity_tools = true`, las herramientas nunca recibirán daño sin importar el porcentaje asignado.
