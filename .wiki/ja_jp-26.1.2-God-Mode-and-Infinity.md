...
| システムパラメータ | 設定値 |
| :--- | :--- |
| **全体無限化ルール** | `ig:dm_infinity_global` |
| **初期状態** | `false`（無効） |
| **ダメージ傍受** | メソッドの`HEAD`でキャンセル（$0$ダメージ適用） |
| **ツールチップ表示** | `✦ UNBREAKABLE`（金色・太字） |
| **優先度** | 絶対優先（すべての倍率計算に先立って判定） |

---

## ⚡ 概要と仕組み

**ゴッドモード（無限）**は、選択したカテゴリのアイテムに完全な無敵性を付与します。ゴッドモードが有効なアイテムは、`ItemStackDurabilityMixin`において耐久度減少イベントが完全に傍受・キャンセルされ、ダメージを受けたり壊れたりすることがなくなります。

### バニラの破壊不可コンポーネントとの違い
* バニラの`Unbreakable`コンポーネントは、コマンド経由で個別のアイテムスタックに付与する必要があります (`/give @p diamond_sword[unbreakable={}]`)。
* ゴッドモードは**ワールド全体・カテゴリ全体**に適用されます。データ編集なしに、ワールド内のすべての道具、武器、防具が自動的に壊れなくなります。

---

## 🛡️ 24のゴッドモードルール

| # | ゲームルールキー | カテゴリ名 | 対象アイテム | 初期値 |
| :-: | :--- | :--- | :--- | :-: |
...
`DurabilityHelper.isInfinite(ServerLevel, ItemStack)`の判定順序：
...
ゴッドモードは、いかなる倍率や使い捨て設定に対しても**絶対的な優先権**を持ちます。`ig:dm_infinity_tools = true`の場合、パーセント設定に関わらず道具は一切ダメージを受けません。
